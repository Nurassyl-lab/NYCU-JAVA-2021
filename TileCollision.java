public class TileCollision{
	...
	public boolean collisionTile(float ax, float ay){
		for (int c = 0; c < 4; c++){
			...
			if (TileMapObj.two_blocks.containsKey(String.valueOf(xt)+ "," + String.valueOf(yt))){
				Block block = TileMapObj.two_blocks.get(String.valueOf(xt)+ "," + String.valueOf(yt));
	...
	private boolean collisionHole(float ax, float ay, float xt, float yt, Block block){
		...
		if(block.isInside(e.getBounds())){
			...
			if(TileMapObj.two_blocks.containsKey(String.valueOf(nextXt)+ "," + String.valueOf(nextYt))){
